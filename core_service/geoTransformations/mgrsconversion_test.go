...
func TestCompleteMGRS(t *testing.T) {
	...
	polConv, _ := geoTransformations.NewPolarStereographicScaleFactor(
		ellipsoidSemiMajorAxis, ellipsoidFlattening, 0, 1.0, geoTransformations.HemisphereNorth, 0, 0,
	)
	upsConv, _ := geoTransformations.NewUPS(ellipsoidSemiMajorAxis, ellipsoidFlattening)
	mgrsConv, _ := geoTransformations.NewMGRS(ellipsoidSemiMajorAxis, ellipsoidFlattening, "WE")
...
func TestMGRSRoundTrip(t *testing.T) {
	mgrs, err := NewMGRS(ellipsoidSemiMajorAxis, ellipsoidFlattening, "WE")
	if err != nil {
...
func fuzzMGRS(data []byte) int {
	...
	mgrs, _ := NewMGRS(ellipsoidSemiMajorAxis, ellipsoidFlattening, "WE")
